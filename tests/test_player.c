...
int main(void)
{
    ...
    for (int i = 0; i < 10; ++i)
        player_update(&player, map, NULL, NULL, NULL, 1.0, 0.0, 0.01);
    assert(fabs(player.x - 2.75) < 0.000001 && player.y == 2.5);

    Player same_time = {2.5, 2.5, 0.0};
    player_update(&same_time, map, NULL, NULL, NULL, 1.0, 0.0, 0.05);
    player_update(&same_time, map, NULL, NULL, NULL, 1.0, 0.0, 0.05);
    assert(fabs(same_time.x - player.x) < 0.000001);
    player_update(&player, map, NULL, NULL, NULL, 0.0, 1.0, 0.05);
    ...
    map[2 * MAP_SIDE + 3] = 1;
    for (int i = 0; i < 20; ++i)
        player_update(&player, map, NULL, NULL, NULL, 1.0, 0.0, 0.05);
